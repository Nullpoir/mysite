...
from app.tasks import scheduled_worker
...
def sns_publish_handler(instance):
    ...
    if (pub_date <= now) and (pub_type == 1) and is_need:
        ...
        if instance.celery_id != None:
            ...
            with disable_signal(signals.post_save, sns_publish_handler, app):
                instance.celery_id = None
                instance.save()

        #ツイート
        twitter_post(instance.title,instance.pk,instance.sns_intro)

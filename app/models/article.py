...
app.objects.published()でpub_dateが
現在時刻より以前のものだけ拾える。
...
class Article(models.Model):
    ...
    def save(self):
        ...
        self.body,self.index = app_convert(self.edit_body)
    ...
    def related_posts(self):
        ...
        query.add(Q(pub_type=1),Q.AND)
        app_list = []
        related_posts_list = app.objects.all().filter(query).order_by('pub_date').reverse().distinct().exclude(pk=self.pk)[:6]
